...
reordering does not corrupt the mapping). Alternate Advancement is also read
from the live [EQL Wiki page](https://eqlwiki.com/Alternate_Advancement): the
bundle vendors a single wiki revision and does not change when that page is
edited, so the extractor overlays a newer wiki revision onto the bundled
abilities instead of treating the bundle hash as AA freshness.
